package dev.sussolino.juicypractice.utils.reflection;
...
public class ReflectionUtil {
    ...
    public void register(String pack, Plugin plugin) {
        for (PlayerCommand c : ReflectionUtil.getCommands(pack)) {
    ...
    @SneakyThrows
    public List<PlayerCommand> getCommands(String packageName) {
        List<PlayerCommand> commands = new ArrayList<>();
        for (Class<?> clazz : getClassesInPackage(packageName)) {
            if (clazz == null) continue;
            if (!clazz.isAnnotationPresent(AntiSocial.class)) continue;
            commands.add((PlayerCommand) clazz.getConstructor().newInstance());
        }
        return commands;
    }
    ...
    public List<Listener> getSpartanChecks(String packageName) {
        ...
        for (Class<?> clazz : getClassesInPackage(packageName)) {
            if (clazz == null) continue;
            if (!clazz.isAnnotationPresent(Spartan.class)) continue;
    ...
    public String path() throws URISyntaxException {
        return new File(Juicy.class.getProtectionDomain().getCodeSource().getLocation()
                .toURI()).getPath();
    }
}